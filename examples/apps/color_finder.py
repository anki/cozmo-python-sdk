...
class ColorFinder(cozmo.annotate.Annotator):
    ...
    def pixel_is_surrounded_by_color(self,i,j):
        ''' If a pixel is surrounded by three pixels of the same color, returns that color. Otherwise, returns None.'''
...
class BlobDetector():
    ...
    def make_blobs_dict(self):
        ''' Constructs a dictionary that maps a blob to the points of the matrix that make up that blob.
                Key : (number,color), where number is self.num_blobs at the time the blob was first created,
                and color is the color of that blob. This way, two distinct blue blobs have two distinct keys.
    ...
    def filter_blobs_dict_by_size(self,n):
        ''' Removes blobs from self.blob_dict with fewer than n points.'''
    ...
    def get_largest_blob_with_color(self,color):
        ''' Returns the largest blob in self.blob_dict of the specified *color*, or None if no such blob exists.'''
    ...
    def get_blob_info(self):
        ''' Returns the approximate center of the blobs
            by averaging the x and y values of the points of each blob.
        '''
        info = {}
        biggest_blobs = []
